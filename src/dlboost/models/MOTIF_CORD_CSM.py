...
class CSE_ContraDyn(nn.Module):
    def __init__(self):
        ...
        # )
        upsample = lambda x: interpolate(x, scale_factor=(1, 2, 2), mode="trilinear")
        self.upsample_third = torch.vmap(lambda x: upsample(upsample(upsample(x))))

    def generate_forward_operator(self, csm_kernels):
        _csm = self.upsample_third(csm_kernels)
        self._csm = _csm / \
...
class MVF_Static(nn.Module):
    ...
    def generate_forward_operator(self, mvf_kernels):
        ...
        _mvf = rearrange(mvf_kernels, "b ph v d h w -> (b ph) v d h w")
...
class SD_RED(nn.Module):
    ...
    def forward(self, kspace_data, kspace_traj, weights):
        ...
        for t in range(self.iterations):
            ...
            loss_dc = 1 / 2 * (torch.abs(torch.view_as_real(kspace_data_estimated) - torch.view_as_real(kspace_data))**2).sum()
    ...
    def update(self, dc_grads, reg_grads, t):
        return pytree.tree_map(
            lambda dc_grad, reg_grad: -(self.gamma
            * dc_grad + self.tau * reg_grad),
            dc_grads,
            reg_grads,
        )
    ...
    def csm_kernel_init(self, image_multi_ch):
        ...
        csm_kernel = csm_kernel / torch.sqrt(torch.sum(torch.abs(csm_kernel) ** 2, dim=1, keepdim=True))
        for i in range(3):
...
class ADAM_RED(SD_RED):
    ...
    def forward(self, kspace_data, kspace_traj, weights):
        ...
        for t in range(self.iterations):
            ...
            kspace_data_estimated = self.forward_model(params)
            loss_dc = 1 / 2 * (torch.abs(torch.view_as_real(kspace_data_estimated) - torch.view_as_real(kspace_data))).mean()
            ...
            updates = pytree.tree_map(lambda dc, reg: dc-self.tau[t]*reg, updates, grad_reg)
