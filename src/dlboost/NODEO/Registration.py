...
from dlboost.NODEO.Network import BrainNet
...
def main(config):
    ...
    )  # two images to be registered must in the same size
    t = time.time()
    df, df_with_grid, warped_moving = registration_main(config, device, moving, fixed)
    runtime = time.time() - t
...
def registration_main(config, device, moving, fixed):
    """
    ...
    # training loop
    scale_factor = torch.tensor(im_shape).to(device).view(1, 3, 1, 1, 1) * 1.0
    ...
    for i in range(config.epoches):
        all_phi = ode_train(
            grid, Tensor(np.arange(config.time_steps)), return_whole_sequence=True
        )
        all_v = all_phi[1:] - all_phi[:-1]
        all_phi = (all_phi + 1.0) / 2.0 * scale_factor  # [-1, 1] -> voxel spacing
        phi = all_phi[-1]
        grid_voxel = (grid + 1.0) / 2.0 * scale_factor  # [-1, 1] -> voxel spacing
...
def registration(device, moving, fixed, verbose=False):
    ...
    ode_train = NeuralODE(Network, "Euler", 0.001).to(device)
    ...
    # Define optimizer
    optimizer = torch.optim.Adam(ode_train.parameters(), lr=0.005, amsgrad=True)
    loss_NCC = NCC(win=21)
    BEST_loss_sim_loss_J = 1000
    for i in range(300):
...
def evaluation(config, device, df, df_with_grid):
    ...
    dice_move2fix = dice(
        warped_seg.unsqueeze(0).unsqueeze(0).detach().cpu().numpy(), fixed_seg, label
    )
...
def save_result(config, df, warped_moving):
    save_nii(
        df.permute(2, 3, 4, 0, 1).detach().cpu().numpy(),
        "%s/df.nii.gz" % (config.savepath),
    )
    save_nii(
        warped_moving.detach().cpu().numpy(), "%s/warped.nii.gz" % (config.savepath)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        dest="fixed",
        default="./data/OAS1_0001_MR1/brain.nii.gz",
    ...
    parser.add_argument(
        ...
        type=str,
        dest="moving",
        default="./data/OAS1_0002_MR1/brain.nii.gz",
    ...
    parser.add_argument(
        ...
        type=str,
        dest="fixed_seg",
        default="./data/OAS1_0001_MR1/brain_aseg.nii.gz",
    ...
    parser.add_argument(
        ...
        type=str,
        dest="moving_seg",
        default="./data/OAS1_0002_MR1/brain_aseg.nii.gz",
        help="moving image segmentation data path",
    )
    # Model configuration
    parser.add_argument(
        "--ds", type=int, dest="ds", default=2, help="specify output downsample times."
    )
    parser.add_argument(
        "--bs", type=int, dest="bs", default=16, help="bottleneck size."
    )
    parser.add_argument(
        "--smoothing_kernel",
        type=str,
    ...
    parser.add_argument(
        "--optimizer", type=str, dest="optimizer", default="Euler", help="Euler or RK."
    )
    parser.add_argument(
        "--STEP_SIZE",
        type=float,
    ...
    parser.add_argument(
        "--NCC_win", type=int, dest="NCC_win", default=21, help="NCC window size"
    )
    parser.add_argument(
        "--lr", type=float, dest="lr", default=0.005, help="Learning rate."
    )
    parser.add_argument(
        "--lambda_J",
        type=int,
    ...
    # Debug
    parser.add_argument(
        "--debug", type=bool, dest="debug", default=False, help="debug mode"
    )
    # Device
    parser.add_argument(
        "--device",
