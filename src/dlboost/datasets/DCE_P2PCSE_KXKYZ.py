...
# %%
class DCE_P2PCSE_KXKYZ_Dataset(Dataset):
    def __init__(self, data, transform=None, mode = "train", patch_size_z = 5, filenames = None) -> None:
        super().__init__()
        ...
        self.ids = [Path(filename).stem for filename in filenames] if isinstance(filenames, list) else Path(filenames).stem
...
class DCE_P2PCSE_KXKYZ(LightningDataModule):
    ...
    def prepare_data(self):
        ...
        if not iou.check_mk_dirs(self.cache_dir/"train"):
            for idx, p in enumerate(self.dat_file_path_list[self.train_scope]):
                ...
                for contra in range(t):
                    dict_data = {k: zarr.array(raw_data[k][contra].numpy(), chunks = (None,None,1,None,None))
                                 for k in self.keys}
            ...
            for idx, p in enumerate(self.dat_file_path_list[self.val_scope]):
                ...
                for contra in range(t):
                    ...
                    if not iou.check_mk_dirs(save_path):
                        zarr.save(save_path, **dict_data)
                        print(filename)
        # if not iou.check_mk_dirs(self.cache_dir/"pred"):
        #     for idx, p in enumerate(self.dat_file_path_list[self.pred_scope]):
        #         dat_file_to_recon = Path(self.data_dir)/p
        #         patient_id = dat_file_to_recon.parent.name
        #         print(patient_id)
        #         raw_data = recon_one_scan(
        ...
        #                          for k in self.keys}
        #             filename = patient_id+"_"+str(contra)+".zarr"
        #             save_path = self.cache_dir/"pred"/filename
        #             if not iou.check_mk_dirs(save_path):
        #                 zarr.save(save_path, **dict_data)
        #                 print(filename)
    ...
    def setup(self, init=False, stage: str = 'train'):
        ...
        # if self.trainer.training:
            ...
            train_datasets = [DCE_P2PCSE_KXKYZ_Dataset(
        ...
        self.val_dataset =DCE_P2PCSE_KXKYZ_Dataset(data, mode = "val", filenames=data_filenames)

        if stage == "predict":
            # data_filenames = glob(str(self.cache_dir/"train/*.zarr"))
            # train_datasets = [DCE_P2PCSE_KXKYZ_Dataset(
            #     zarr.open(filename, mode='r'), patch_size_z = 80, filenames=filename) for filename in data_filenames]
            # self.pred_train_dataset = torch.utils.data.ConcatDataset(train_datasets)
            # data_filenames = glob(str(self.cache_dir/"val/*.zarr"))
            # data  = [zarr.open(filename, mode='r') for filename in data_filenames]
            # data_filenames = glob(str(self.cache_dir/"val/ONC-DCE-004_*.zarr"))
            # data  = [zarr.open(filename, mode='r') for filename in data_filenames]
            # self.pred_val_dataset =DCE_P2PCSE_KXKYZ_Dataset(data, mode = "val", filenames=data_filenames)
            data_filenames = glob(str(self.cache_dir/"val/*.zarr"))
            data  = [zarr.open(filename, mode='r') for filename in data_filenames]
            self.pred_dataset = DCE_P2PCSE_KXKYZ_Dataset(data, mode="val", filenames=data_filenames)
            # val_datasets = [DCE_P2PCSE_KXKYZ_Dataset(
            #     zarr.open(filename, mode='r'), mode = "val", filename=filename) for filename in data_filenames]
            # self.pred_val_dataset = torch.utils.data.ConcatDataset(val_datasets)
    ...
    def predict_dataloader(self):
        return DataLoader(self.pred_dataset, batch_size=1, num_workers=0, collate_fn=lambda x: x, shuffle=False)
