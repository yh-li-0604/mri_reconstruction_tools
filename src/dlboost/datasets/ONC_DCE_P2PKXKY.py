...
class ONC_DCE_P2PKXKY(LightningDataModule):
    def __init__(
        ...
        self.keys = ["kspace_data_z", "kspace_data_z_compensated", "kspace_traj"]
    ...
    def setup(self, init=False, stage: str = 'train'):
        if stage == 'train' or stage == 'continue':
            train_iter = zip(*[zarr.open(self.cache_dir/"train"/(k+".zarr"), mode='r') for k in ["kspace_data_z_compensated_fixed", "kspace_data_z_fixed", "kspace_traj_fixed", "kspace_data_z_compensated_moved", "kspace_data_z_moved", "kspace_traj_moved"]])
            train_data_list = [dict(kspace_data_compensated_fixed = kcf, 
                                ...
                                kspace_traj_moved = ktm) for kcf, kf, ktf, kcm, km, ktm in train_iter]
            train_transforms = Compose([
                ...
                ToTensord(device=torch.device('cpu'), keys=[
                    'kspace_data_compensated_fixed', 'kspace_data_fixed', 'kspace_traj_fixed',
                    'kspace_data_compensated_moved', 'kspace_data_moved', 'kspace_traj_moved',
                ]),
