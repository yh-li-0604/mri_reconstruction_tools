...
class ReconRegis(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for fixed_ph in range(image_init.shape[1]):
            # image_init_list = torch.unbind(image_init.clone(), dim=1)
            # image_init_list[fixed_ph] = torch.zeros_like(image_init_list[fixed_ph])
            image_1blank = [image_init[:, ph, ...] if ph != fixed_ph else torch.zeros_like(
                image_init[:, fixed_ph, ...]) for ph in range(image_init.shape[1])]
            image_input = torch.stack(image_1blank, dim=1)
            image_recon = self.forward(image_input)
        # if self.global_step %  == 0:
            for moved_ph in range(image_init.shape[1]):
                if fixed_ph == 0 and moved_ph == 0:
                    for i in range(image_init.shape[1]):
                        to_png(self.trainer.default_root_dir+f'/image_init_ph{i}.png',
                               image_input[0, i, 1, :, :], vmin=0, vmax=2)
                ...
                loss_f2m = self.__training_step_recon(image_recon=wrap_f2m,
                                                      kspace_traj=kspace_traj_moved,
                                                      kspace_data=kspace_data_moved)
                recon_loss = loss_m2f + 0.2*loss_f2m
