...
class ReconRegis(pl.LightningModule):
        ...
        if self.is_optimize_regis and self.global_step%2==0:
            ...
            for fixed_ph, moved_ph in combinations_with_replacement(range(image_recon.shape[1]), 2):
                # if fixed_ph == moved_ph:
                    ...
                    # recon_loss = self.__training_step_recon(image_recon=image_recon_self,
                ...
                if fixed_ph != moved_ph:
                    ...
                    self.manual_backward(regis_loss+recon_loss, retain_graph=True)
            regis_opt.step()
    ...
    def forward_ch(self, kspace_data_compensated, kspace_traj, cse):
        ...
        for kspace_data_ch,cse_ch in zip(kspace_data_compensated ,cse):
            ...
            # zarr.save(f'tests/DeCoLearn_KXKYZ/image_recon_{ch}.zarr', recon.abs().numpy(force=True))
            # zarr.save(f'tests/image_init_DeCoLearn_KXKYZ_{ch}.zarr', init.abs().numpy(force=True))
            print(ch)
            ch+=1
        # image_recon = torch.sum(torch.cat(image_recon_list),dim=0)
        # image_init = torch.sum(torch.cat(image_init_list),dim=0)
        zarr.save(f'tests/DeCoLearn_KXKYZ/image_recon_.zarr', image_recon.abs().numpy(force=True))
        zarr.save(f'tests/DeCoLearn_KXKYZ/image_init_.zarr', image_init.abs().numpy(force=True))
    ...
    def forward_step(self, kspace_data_compensated, kspace_traj,cse):
        image_init = self.nufft_adj_fn(kspace_data_compensated, kspace_traj)
        x_ = complex_as_real_2ch(image_init)
        x_ = sliding_window_inference(
                x_, roi_size=self.patch_size, sw_device=torch.device('cpu'), device=torch.device('cpu'),
                sw_batch_size=8, predictor=self.recon_module, mode='gaussian')
        image_recon = real_2ch_as_complex(x_,c=5)
