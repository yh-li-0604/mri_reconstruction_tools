...
class Recon(pl.LightningModule):
    ...
    def calculate_recon_loss(self, image_recon, kspace_traj, kspace_data, weight):
        ...
        freq_dist = torch.abs(kspace_data_estimated - kspace_data)
        loss_not_reduced = freq_dist
        # loss_not_reduced = \
    ...
    def predict_step(self, batch: Any, batch_idx: int = 0, dataloader_idx: int = 0, device = torch.device("cuda")) -> Any:
        return predict_step(batch, self.nufft_adj, self, self.patch_size, device = device)
