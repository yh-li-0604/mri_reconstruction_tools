...
class Recon(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        kspace_density_compensation_fixed, kspace_density_compensation_moved = batch['kspace_density_compensation_fixed'], batch['kspace_density_compensation_moved']
        ...
        kspace_density_compensation = torch.cat([kspace_density_compensation_fixed, kspace_density_compensation_moved], dim=1)*10000
        print(kspace_density_compensation.std())
    ...
    def __training_step_recon(self, image_recon, kspace_traj, kspace_data, kspace_density_compensation):
        kspace_data_estimated = self.nufft(
            image_recon, kspace_traj)
        print(torch.mean(kspace_density_compensation))
        loss = \
            self.recon_loss_fn(kspace_data_estimated.real*kspace_density_compensation, kspace_data.real*kspace_density_compensation) +\
            self.recon_loss_fn(kspace_data_estimated.imag*kspace_density_compensation, kspace_data.imag*kspace_density_compensation)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        zarr.save('tests/image_recon_P2PKXKY_RandOut_WeightedK.zarr', image_recon)
        zarr.save('tests/image_init_P2PKXKY_RandOut_WeightedK.zarr', image_init)
        breakpoint()
