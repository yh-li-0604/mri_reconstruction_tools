...
# import torchopt
from dlboost.models import XDGRASP
from dlboost.utils import to_png
from dlboost.utils.tensor_utils import formap
...
def forward_patch(
    ...
    csm: [ch, z, h, w]
    """
    image, init = recon_module(
        kspace_data.to(inference_device),
        kspace_data_compensated.to(inference_device),
        kspace_traj.to(inference_device),
        csm.to(inference_device),
    )
    return image.to(storage_device), init.to(storage_device)
...
def forward_all(
    ...
    f = formap(
        lambda k, kc, s: forward_patch(
            k, kc, kspace_traj, s, recon_module, inference_device, storage_device
        ),
        in_dims=(3, 3, 2),
        out_dims=(3, 3),
        batch_size=patch_size[0],
    )
    return f(kspace_data, kspace_data_compensated, csm)
...
class Recon(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.recon_module = XDGRASP(patch_size, nufft_im_size, lambda1, lambda2)
        self.lr = lr
    ...
    def n2n_step(
        self, kspace_data_a, kspace_traj_a, kspace_data_b, kspace_traj_b, ref_idx
    ):
        ...
        weight = torch.cat([weight.flip(0), weight], dim=0)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        zarr.save(
            ...
            + "/image_recon.zarr",
            image[0, 0, :, 35:45].abs().numpy(force=True),
        )
        zarr.save(
            self.trainer.default_root_dir
            +
            ...
            f"/epoch_{self.trainer.current_epoch}"
            + "/mvf.zarr",
            mvf[0, :, :, :, 35:45].abs().numpy(force=True),
        )
        zarr.save(
            self.trainer.default_root_dir
            +
            ...
            f"/epoch_{self.trainer.current_epoch}"
            + "/csm.zarr",
            csm[0, :, :, :, 35:45].abs().numpy(force=True),
        )
        print(
            "Save image, csm and mvf to "
            + self.trainer.default_root_dir
            + f"/epoch_{self.trainer.current_epoch}"
        )
        for ch in [0, 3, 5]:
    ...
    def predict_step(
        ...
        t, ph, ch, z, sp = batch["kspace_data"].shape
        cse = batch["cse"].expand(t, ch, z, -1, -1)
