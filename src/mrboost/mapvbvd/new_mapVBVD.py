...
def loop_mdh_read(fid, version, Nscans, scan, measOffset, measLength, print_prog=True):
    ...
    dmaIdx = (
        np.array(np.arange(29, 33) + 20 * isVD, dtype=np.uint8) - 1
    )  # to correct DMA length using NCol and NCha
    if isVD:
        dmaOff = szScanHeader
    ...
    if print_prog:
        ...
        t = tqdm(
            ...
            unit_scale=True,
            unit_divisor=1024,
            desc="Scan %d/%d, read all mdhs" % (scan + 1, Nscans),
            leave=False,
            file=stdout,
        )
...
def evalMDH(mdh_blob, version):
    if version == "vd":
        isVD = True
        mdh_blob = np.concatenate(
            (mdh_blob[0:20, :], mdh_blob[40:, :]), axis=0
        )  # remove 20 unnecessary bytes
    else:
        isVD = False
    ...
    mdh = MDH(
        ...
        data_uint16[:, 26 - 1],
    )  # ushKSpaceCentrePartitionNo 79 :  80
    ...
    mask = MASK(
        ...
        np.minimum(
            mdh.aulEvalInfoMask[:, 1] & 2 ** (53 - 32), 1
        ),  # WTC modified this as the original matlab code didn't make sense
...
def mapVBVD(filename, quiet=False, **kwargs):
    ...
    for s in range(NScans):
        ...
        if bReadMDH:
            ...
            currTwixObj.update(
                {"refscan_phasestab_ref0": mytmo("refscan_phasestab_ref0")}
            )
            currTwixObj.update(
                {"refscan_phasestab_ref1": mytmo("refscan_phasestab_ref1")}
            )
            ...
            mdh_blob, filePos, isEOF = loop_mdh_read(
                fid,
                version,
                NScans,
                s,
...
class myAttrDict(AttrDict):
    ...
    def search_header_for_val(self, top_lvl, keys, **kwargs):
        ...
        Args:
            ...
            regex (optional): Search using regex or for exact strings.
        """
        keys = self["hdr"].search_for_keys(
            keys, print_flag=False, top_lvl=top_lvl, **kwargs
        )
