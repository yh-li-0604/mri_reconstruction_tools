...
def evalMDH(mdh_blob, version):
    ...
    class MASK:
        ...
        MDH_ONLINE                   = np.minimum(evalInfoMask1 & 2 ** 3, 1)                                                                                                             
        MDH_OFFLINE                  = np.minimum(evalInfoMask1 & 2 ** 4, 1)                                                                                                             
        MDH_SYNCDATA                 = np.minimum(evalInfoMask1 & 2 ** 5, 1)                                                                                                             
        MDH_LASTSCANINCONCAT         = np.minimum(evalInfoMask1 & 2 ** 8, 1)                                                                                                             
        MDH_RAWDATACORRECTION        = np.minimum(evalInfoMask1 & 2 ** 10, 1)                                                                                                            
        MDH_LASTSCANINMEAS           = np.minimum(evalInfoMask1 & 2 ** 11, 1)                                                                                                            
        MDH_SCANSCALEFACTOR          = np.minimum(evalInfoMask1 & 2 ** 12, 1)                                                                                                            
        MDH_2NDHADAMARPULSE          = np.minimum(evalInfoMask1 & 2 ** 13, 1)                                                                                                            
        MDH_REFPHASESTABSCAN         = np.minimum(evalInfoMask1 & 2 ** 14, 1)                                                                                                            
        MDH_PHASESTABSCAN            = np.minimum(evalInfoMask1 & 2 ** 15, 1)                                                                                                            
        MDH_D3FFT                    = np.minimum(evalInfoMask1 & 2 ** 16, 1)                                                                                                            
        MDH_SIGNREV                  = np.minimum(evalInfoMask1 & 2 ** 17, 1)                                                                                                            
        MDH_PHASEFFT                 = np.minimum(evalInfoMask1 & 2 ** 18, 1)                                                                                                            
        MDH_SWAPPED                  = np.minimum(evalInfoMask1 & 2 ** 19, 1)                                                                                                            
        MDH_POSTSHAREDLINE           = np.minimum(evalInfoMask1 & 2 ** 20, 1)                                                                                                            
        ...
        MDH_SHARENOW                 = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # all lines are acquired from the actual and previous e.g. phases                                                      =             ==
        MDH_LASTMEASUREDLINE         = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # indicates that the current line is the last measured line of all succeeding e.g. phases                        =             = = 
        MDH_FIRSTSCANINSLICE         = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # indicates first scan in slice                                                                                  = any(setFlags= = needed for time stamps)
        MDH_LASTSCANINSLICE          = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # indicates  last scan in slice                                                                                  = any(setFlags= = needed for time stamps)
        MDH_TREFFECTIVEBEGIN         = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # indicates the begin time stamp for TReff                                                                       = any(setFlags= = triggered measurement)
        MDH_TREFFECTIVEEND           = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # indicates the   end time stamp for TReff (triggered measurement)                                                 =             = = 

        MDH_MDS_REF_POSITION         = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # indicates the reference position for move during scan images (must be set once per slice/partition in MDS mode)=             = = 
        MDH_SLC_AVERAGED             = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # indicates avveraged slice for slice partial averaging scheme                                                   =             = = 
        MDH_TAGFLAG1                 = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # adjust scan                                                                                                    =             = = 
        MDH_CT_NORMALIZE             = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Marks scans used to calculate correction maps for TimCT-Prescan normalize                                          =             = = 
        MDH_SCAN_FIRST               = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Marks the first scan of a particular map                                                                           =             = = 
        MDH_SCAN_LAST                = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Marks the last scan of a particular map                                                                            =             = = 
        MDH_FIRST_SCAN_IN_BLADE      = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Marks the first line of a blade                                                                                    =             = = 
        MDH_LAST_SCAN_IN_BLADE       = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Marks the last line of a blade                                                                                     =             = = 
        MDH_LAST_BLADE_IN_TR         = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Set for all lines of the last BLADE in each TR interval                                                            =             = = 
        MDH_PACE                     = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Distinguishes PACE scans from non PACE scans.                                                                      =             = = 
        MDH_RETRO_LASTPHASE          = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Marks the last phase in a heartbeat                                                                                =             = = 
        MDH_RETRO_ENDOFMEAS          = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Marks an ADC at the end of the measurement                                                                         =             = = 
        MDH_RETRO_REPEATTHISHEARTBEAT= np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Repeat the current heartbeat when this bit is found                                                                =             = = 
        MDH_RETRO_REPEATPREVHEARTBEAT= np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Repeat the previous heartbeat when this bit is found                                                               =             = = 
        MDH_RETRO_ABORTSCANNOW       = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Just abort everything                                                                                              =             = = 
        MDH_RETRO_LASTHEARTBEAT      = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # This adc is from the last heartbeat (a dummy)                                                                      =             = = 
        MDH_RETRO_DUMMYSCAN          = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # This adc is just a dummy scan, throw it away                                                                       =             = = 
        MDH_RETRO_ARRDETDISABLED     = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Disable all arrhythmia detection when this bit is found                                                            =             = = 
        MDH_B1_CONTROLLOOP           = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Marks the readout as to be used for B1 Control Loop                                                                =             = = 
        MDH_SKIP_ONLINE_PHASCOR      = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Marks scans not to be online phase corrected, even if online phase correction is switched on                       =             = = 
        MDH_SKIP_REGRIDDING          = np.minimum(evalInfoMask1 & 2 ** 26, 1)      # Marks scans not to be regridded, even if regridding is switched on                                                 =             = = 
